...
public class SearchLegacyConnectionsColleague : IColleague
{
    ...
    private readonly ILogger<SearchLegacyConnectionsColleague> _logger;

    /// <summary>
    /// Creates an instance of <see cref="SearchLegacyConnectionsColleague"/>
    /// </summary>
    ...
    public SearchLegacyConnectionsColleague(
        IMediator mediator,
        ...
        ILogger<SearchLegacyConnectionsColleague> logger)
    {
    ...
    public async Task<object> Receive(string serviceKey, object? payloadData)
    {
        ...
        var searchCriteria = payloadData as SearchCriteria;

        if (string.IsNullOrEmpty(searchCriteria?.Criteria))
        {
            throw new ConflictException(ErrorNames.PurgeInProgress, "Must provide valid search criteria.");
        }
        ...
        var legacyConnections = await _legacyService.SearchLegacyConnectionsAsync(filter, searchCriteria);
        var totalRecords = _legacyService.GetLegacyConnectionsCountWithCriteria(searchCriteria);
